...
public class IdentifyingCode {
    ...
    public static final int IMG_WIDTH = AUTHCODE_LENGTH * (SINGLECODE_WIDTH + SINGLECODE_GAP);  //验证码宽度
    public static final int IMG_HEIGHT = SINGLECODE_HEIGHT;  //验证码高度 
    ...
    public static String getAuthCode() {  
        String authCode = "";  
        String str = "0123456789qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLMNBVCXZ";
        for(int i = 0; i < AUTHCODE_LENGTH; i++) {  
            authCode += (str.charAt(new Random().nextInt(62)));  
        }  
        return authCode;  
    }  
    ...
    public static BufferedImage getAuthImg(String authCode) {  
        ...
        BufferedImage img = new BufferedImage(IMG_WIDTH+5, 30, BufferedImage.TYPE_INT_BGR);  
        //得到图片上的一个画笔  
        Graphics g = img.getGraphics();  
        //设置画笔的颜色，用来做背景色  
        g.setColor(Color.YELLOW);  
        //用画笔来填充一个矩形，矩形的左上角坐标，宽，高  
        g.fillRect(0, 0, IMG_WIDTH+5, 30);  
        //将画笔颜色设置为黑色，用来写字  
